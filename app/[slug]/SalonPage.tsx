...
export default function SalonPage({ salon, barbers, services, slug }: Props) {
  ...
  const mapHref = mapUrl
    ? mapUrl.replace('maps/embed?pb=', 'maps?q=').replace('/maps/embed', '/maps')
      ...
      <main>
          ...
          <div className="mx-auto max-w-6xl px-4 py-20 lg:px-6">
            <div className="flex flex-wrap items-end justify-between gap-6 reveal">
              <div>
                ...
                <p className="mt-2 text-white/50 leading-relaxed">عروض موسمية وتجارب VIP — اسأل في الصالون عن أحدث التفاصيل.</p>
              </div>
              <button type="button" onClick={() => openBooking()} className="btn-gold w-full rounded-xl px-6 py-3 text-sm font-extrabold text-black sm:w-auto">احجز الآن</button>
            </div>
            <div className="mt-12 grid gap-5 md:grid-cols-3">
              {[
                { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>, label: 'حجز مميز', title: 'أولوية المواعيد', desc: 'احجز مسبقًا لتحصل على وقتك المفضل وخدمة مرتّبة بلا انتظار.' },
                { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456Z"/>, label: 'عناية VIP', title: 'لمسة فاخرة', desc: 'تفاصيل دقيقة واهتمام كامل من أول دقيقة حتى آخر تشذيب.' },
                { icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75M12 20.25c1.472 0 2.882.265 4.185.75M18.75 4.97A48.416 48.416 0 0 0 12 4.5c-2.291 0-4.545.16-6.75.47m13.5 0c1.01.143 2.01.317 3 .52m-3-.52 2.62 10.726c.122.499-.106 1.028-.589 1.202a5.988 5.988 0 0 1-2.031.352 5.988 5.988 0 0 1-2.031-.352c-.483-.174-.711-.703-.59-1.202L18.75 4.971Zm-16.5.52c.99.203 1.99.377 3 .52m0 0 2.62 10.726c.122.499-.106 1.028-.589 1.202a5.989 5.989 0 0 1-2.031.352 5.989 5.989 0 0 1-2.031-.352c-.483-.174-.711-.703-.59-1.202L5.25 5.491Z"/>, label: 'عروض موسمية', title: 'باقات مختارة', desc: 'تتوفر عروض حسب الموسم—اسألنا عن الباقات المتاحة اليوم.' },
              ].map((item, i) => (
                <article key={i} className={`glass-card reveal reveal-d${i+1} rounded-3xl p-7`}>
                  <div className="mb-5 text-gold"><svg className="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">{item.icon}</svg></div>
                  <div className="sec-label" style={{ marginBottom: '.4rem' }}>{item.label}</div>
                  <div className="text-xl font-extrabold mb-3">{item.title}</div>
        ...
        <section id="packages" className="scroll-mt-24 bg-ink">
          <div className="mx-auto max-w-6xl px-4 py-20 lg:px-6">
            <div className="flex flex-wrap items-end justify-between gap-6 reveal">
              <div>
                <div className="sec-label">الأكثر طلبًا</div>
                <h2 className="text-3xl font-extrabold tracking-tight lg:text-4xl">الباقات</h2>
                <p className="mt-2 text-white/50 leading-relaxed">اختيار سريع لأكثر الخدمات طلبًا—بأسلوب فاخر.</p>
              </div>
